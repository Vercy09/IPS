...
FPSManager::FPSManager(GLFWwindow *window, int theTargetFps, float theReportInterval, std::string theWindowTitle, unsigned int* activeCount)
{
		...
		if ((frameEndTime - lastReportTime) > reportInterval)
		{
			...
			if (verbose)
			{
				...
				if (windowTitle != "NONE")
				{
					...
					std::string pCount = std::to_string(*particleCount);

					// Append the FPS value to the window title details
					std::string tempWindowTitle = windowTitle + " | FPS: " + fpsString + " | Particle count: " + pCount;
